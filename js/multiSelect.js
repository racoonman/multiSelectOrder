...
!function($) {
    ...
    MultiSelect.prototype = {
        ...
        init: function() {
            ...
            if (element.next('.ms-container').length === 0) {
                element.css({position: 'absolute', left: '-9999px'});
                element.attr('id', element.attr('id') ? element.attr('id') : Math.ceil(Math.random() * 1000) + 'multiselect');
                this.$container.attr('id', 'ms-' + element.attr('id'));
            ...
            var selectedValues = element.find('option:selected').map(function() {
                return $(this).val();
            }).get();
